...
public class RemoveFromCartSevlet extends HttpServlet {
    ...
    private void process(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		LOGGER.info("servlet /removeFromCart");
        ...
        LOGGER.info("User " + user.getUsername() + " remove shopping items from carts");
		DatabaseManager dbm = new DatabaseManager(dataSource);
		Connection conn = dbm.getConnection();
		try{
			// Remove shopping items from carts
			ShoppingCartDao.updateCart(conn, user.getId(), isbn, "delete");
			ShoppingCart shoppingCart = ShoppingCartDao.retrieveCart(conn, user.getId());
			request.setAttribute("cart", shoppingCart);
        } catch (SQLException e) {
            LOGGER.log(Level.SEVERE, "error remove shopping items from shopping cart", e);
        } finally {
